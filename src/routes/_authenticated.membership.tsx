    ...
    <div className="space-y-6">
      ...
      >
        <DialogContent>
          ...
          {flow.kind === "failed" && flow.message && (
            <p className="rounded-md border border-destructive/40 bg-destructive/10 p-2 text-xs font-mono text-destructive">
              {flow.message}
            </p>
          )}
          <DialogFooter>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Unable to connect to Pi Network. Please try again.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button onClick={() => setFlow({ kind: "idle" })}>Close</Button>
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      ...
      <span className={highlight ? "font-extrabold text-gradient-gold" : "font-semibold"}>{value}</span>
    </div>
  );
}
